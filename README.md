...
按章节拆分的多页可视化站点：首页 `index.html` 是章节目录，每个技术章节是一个独立页面，放在 `chapters/` 目录下，顶部搜索框支持跨章节全局检索。AI 相关内容已按 `LLM 基础与 Prompt`、`RAG 知识增强`、`AI Agent 自动执行`、`AI 工具生态与能力扩展`、`AI 测试与评测`、`LLMOps 工程化工具` 拆分。
